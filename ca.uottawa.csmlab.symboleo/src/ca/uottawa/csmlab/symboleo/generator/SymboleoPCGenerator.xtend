...
class SymboleoPCGenerator extends SymboleoGenerator {
	...
	def static String eventToSituation (String event) {
		switch (event) {
			...
			case "revokedParty": return "unassigns"	
	...
	def void compileContract(IFileSystemAccess2 fsa, Model model) {
		...
		MODULE «model.contractName» («generateRoleInstances2()»«pcParameters.join(', ')»«eventEnv.length()> 0 ? ','» «eventEnv.join(', ')»)
			...
			VAR
				...
				«AssignWorkdef(model)»
		'''
		fsa.generateFile("./SymboleoPC/contracts/" + model.contractName + ".smv", code)
	}	
...
//				parameters = parameters.substring(0, parameters.length()-2)
	...
	def void generateProblemFile(IFileSystemAccess2 fsa, Model model) {
		...
		'''
		fsa.generateFile("./SymboleoPC/contracts/problemFile_" + model.contractName + ".txt", code)
	}
	...
	override def void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
